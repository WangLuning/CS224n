...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size = embed_size, hidden_size = self.hidden_size, dropout = dropout_rate, bidirectional = True)
        self.decoder = nn.LSTMCell(embed_size + self.hidden_size, self.hidden_size, bias = True)

        # these are all the weights to be trained
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), 1) # from (2,b,h) to (b,2*h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # time split means, batch is maintained, we start with the first token of every batch, that is also why we pad it to same length
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # previously enc_hiddens_proj is encoder part of e_t, we now multiply decoder part to it
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2)   # shape (b, src_len) so it mimics dist of attention
        ...
        # it means, we use attention as well as lstm decoder hidden state, to compute the final outcome
        U_t = torch.cat((a_t, dec_hidden), dim = 1)    # shape (b, 3h)
