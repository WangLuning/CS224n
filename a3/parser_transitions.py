...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        # the two stacks face different directions......
        if transition == 'S':
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        mark_removed = []
        # select batch size sentences for parsing in each batch
        transitions = model.predict(unfinished_parses[:batch_size])
        for i, transition in enumerate(transitions):
            unfinished_parses[i].parse_step(transition)
            # need to mark as removed first, not actually remove it directly
            # because if removed, the index i for the next parse is wrongly pointed
            if len(unfinished_parses[i].buffer) == 0 and len(unfinished_parses[i].stack) == 1:
                mark_removed.append(i)
        mark_removed.reverse()
        for i in mark_removed:
